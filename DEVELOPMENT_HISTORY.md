...
### `[favorites-feat]` 2026-08-14 | feat(web): 引入歌曲红心收藏与播放列表「全部/我的收藏」分类切换功能
* **歌单与歌曲收藏（Favorites Collection）系统落地**：
  - **主界面红心点亮与心跳动效**：在唱片信息右侧新增红心收藏按钮（`#btn-favorite-current`），播放时自动感知收藏状态，点击触发 `heartBounce` 心跳缩放动画与平滑高亮；
  - **歌单侧边栏「全部歌曲」与「我的收藏」Tab**：在抽屉顶部提供精致的毛玻璃分组选择器与数字角标，支持收藏列表与全局歌曲无缝一键切换；
  - **单曲快捷收藏与空状态展示**：歌曲列表项（`track-item`）右侧增加悬停快捷红心收藏；收藏为空时展示精致的插画与操作引导；
  - **LocalStorage 数据持久化与跨歌单收藏**：已收藏的歌曲对象持久化保存在本地，即使用户切换导入新的外部网易云歌单，红心收藏也绝不丢失。
