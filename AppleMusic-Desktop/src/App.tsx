        ...
        <main className="flex-1 h-[calc(100vh-44px-72px)] overflow-y-auto px-8 py-7 max-w-[1120px] mx-auto w-full">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              animate="animate"
              exit="exit"
              className="space-y-8 pb-12 min-w-0"
            >
              <Suspense
                fallback={
              ...
              >
                {activeView === "lyrics" ? (
                  ...
                  <>
                    ...
                    >
                      <div className="max-w-xl space-y-3.5 relative z-10 min-w-0">
                        <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/20 dark:bg-white/20 backdrop-blur-md text-[11px] font-semibold tracking-wide uppercase">
                          <Sparkles className="w-3.5 h-3.5 flex-shrink-0" /> 今日聚焦 · 杜比全景声
                        </span>
                        <h1 className="text-3xl font-extrabold apple-title leading-tight line-clamp-2 break-words">
                          {spatialPlaylist?.title || "Apple Spatial Audio 空间音频精选"}
                        </h1>
                        <p className="text-xs sm:text-sm text-white/85 leading-relaxed line-clamp-2 break-words max-w-lg">
                        ...
                        <div className="pt-2 flex items-center gap-3 flex-wrap">
                          <Button
                            ...
                            className="bg-white text-black hover:bg-white/95 shadow-md px-5 font-semibold text-xs h-8.5 flex-shrink-0"
                          >
                            {isPlaying ? (
                              <>
                                <Pause className="w-3.5 h-3.5 fill-current mr-1 flex-shrink-0" /> 暂停播放
                              </>
                            ) : (
                              <>
                                <Play className="w-3.5 h-3.5 fill-current ml-0.5 mr-1 flex-shrink-0" /> 立即播放 ({spatialPlaylist?.tracks?.length || 0} 首)
                              </>
                            )}
                          </Button>
                          <Button
                            ...
                            variant="secondary"
                            className="bg-black/20 hover:bg-black/30 text-white border border-white/20 px-4 font-semibold text-xs h-8.5 flex-shrink-0"
                          >
                            搜索探索
                          </Button>
                        </div>
                    ...
                    <section className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h2 className="text-lg font-bold apple-title flex items-center gap-2">
                          <Disc3 className="w-4 h-4 text-[#FA2D48] flex-shrink-0" /> 精选曲目推荐
                        </h2>
                          ...
                          {spatialPlaylist?.tracks?.map((track, idx) => {
                            ...
                            return (
                              <motion.div key={track.id} variants={staggerItem} className="min-w-0">
                                ...
                                >
                                  <div className="flex items-center gap-3 overflow-hidden min-w-0 flex-1 pr-3">
                    ...
                    <section className="space-y-4">
                      <div className="flex items-center justify-between">
                        <h2 className="text-lg font-bold apple-title flex items-center gap-2">
                          <Flame className="w-4 h-4 text-[#FA2D48] flex-shrink-0" /> 热门官方歌单
                        </h2>
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => setActiveView("search")}
                          className="apple-caption hover:text-neutral-900 dark:hover:text-white flex items-center gap-1 flex-shrink-0"
                        >
                          <span>查看全部</span>
                      ...
                      >
                        {[
                          ...
                          <motion.div key={idx} variants={staggerItem} className="min-w-0">
                            <div
                              onClick={() => setActiveView("album", "17910751956")}
                              className="bento-card p-2.5 space-y-2.5 group cursor-pointer hover:shadow-lg transition-all overflow-hidden"
                            >
